...
import { getDisplayAssignmentCodes } from '@/lib/customRoles';
...
export const LiveActivities = ({ competitionId }: LiveActivitiesProps) => {
  const { childActivities, personsInActivities, liveActivities, wcif } = useCommon(competitionId);
  const now = useNow();
  const displayAssignmentCodes = getDisplayAssignmentCodes(
    wcif,
    personsInActivities?.map((person) => person.assignment?.assignmentCode) ?? [],
  );
  ...
  return (
    <div className="grid grid-cols-12 space-y-2">
      ...
      {displayAssignmentCodes.map((assignmentCode) => (
