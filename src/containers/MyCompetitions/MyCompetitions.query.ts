...
export const loadUserCompetitions = async (userId: string) => {
  const res = normalizeUserCompetitions(await fetchUserWithCompetitions(userId));
...
export const useMyCompetitionsQuery = (userId?: number, options: { enabled?: boolean } = {}) => {
  const { data, ...props } = useQuery<UserCompsResponse, string>({
    ...
    queryFn: () => loadUserCompetitions(userId!.toString()),
