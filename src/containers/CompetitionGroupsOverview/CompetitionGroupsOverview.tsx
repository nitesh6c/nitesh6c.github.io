...
import { getStaffAssignmentShortLabel, isStaffLikeAssignmentCode } from '@/lib/customRoles';
...
const staffingAssignmentToText = (
  { assignmentCode, activity },
  wcif: ReturnType<typeof useWCIF>['wcif'],
) => `${getStaffAssignmentShortLabel(assignmentCode, wcif)}${groupNumber(activity)}`;
    ...
    (person) => {
      ...
      wcif?.events.forEach((event) => {
        ...
        const staffingAssignments = assignmentsForEvent.filter(({ assignmentCode }) =>
          isStaffLikeAssignmentCode(assignmentCode),
        );
        ...
        obj.staffing[event.id.toString() + '_staff'] = staffingAssignments
          .map((assignment) => staffingAssignmentToText(assignment, wcif))
          .join(',');
      });

      return obj;
    },
    [memoizedGroupActivitiesForRound, wcif?.events, wcif],
  );
              ...
              <Fragment key={stage.id}>
                ...
                {assignments
                  ...
                  .map((person) => {
                    ...
                    const staffingAssignments = personAssignments.filter(({ assignmentCode }) =>
                      isStaffLikeAssignmentCode(assignmentCode),
                    );
                        ...
                        {wcif?.events.map((event) => {
                          ...
                          return (
                            <AssignmentCodeCell
                              ...
                              {staffingAssignment &&
                                staffingAssignmentToText(staffingAssignment, wcif)}
                            </AssignmentCodeCell>
                          );
                        })}
                      </tr>
