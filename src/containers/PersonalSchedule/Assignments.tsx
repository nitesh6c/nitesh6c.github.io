...
const isSameScheduleActivityGroup = (
  leftActivityCode: string | undefined,
  rightActivityCode: string | undefined,
  leftActivityId?: number,
  rightActivityId?: number,
) => {
  if (!leftActivityCode || !rightActivityCode) {
    return false;
  }

  if (leftActivityId != null && rightActivityId != null && leftActivityId === rightActivityId) {
    return true;
  }

  const left = parseActivityCodeFlexible(leftActivityCode);
  const right = parseActivityCodeFlexible(rightActivityCode);
                  ...
                  {(collapsed ? [] : assignments)
                    ...
                    .map(({ date, assignment }, index, sortedAssignments) => {
                      ...
                      const previousAssignmentActivityCode =
                        previousAssignment?.activity?.activityCode;
                      const nextAssignmentActivityCode = nextAssignment?.activity?.activityCode;
                      ...
                      const previousActivityIsSameRoundAttempt =
                        ...
                        isSameScheduleActivityGroup(
                          ...
                          previousAssignmentActivityCode,
                          assignment.activity?.id,
                          previousAssignment?.activity?.id,
                        );
