...
export default function Settings() {
  ...
  const { preferences } = useCustomRoleColorPreferences();
  const savedCustomRoleIds = Object.keys(preferences);
  ...
  return (
    <Container className="px-4 py-8">
      <div className="space-y-6">
        ...
        {savedCustomRoleIds.length > 0 && (
          <div className="space-y-4 p-6 bg-panel rounded-lg shadow-md shadow-tertiary-dark">
            <div className="space-y-2">
              <h2 className="type-heading">Custom assignment colors</h2>
              <p className="type-body-sm text-subtle">
                Colors you have chosen for custom roles. Open a competition personal schedule and
                expand &ldquo;Custom assignment colors&rdquo; at the bottom to change them.
              </p>
            </div>
            <ul className="space-y-1 type-meta">
              {savedCustomRoleIds.map((roleId) => (
                <li key={roleId}>{getAssignmentDisplayName(roleId, undefined)}</li>
              ))}
            </ul>
          </div>
        )}
